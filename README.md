...
CodeReview GitHub 이슈를 AI로 분석하여 자동으로 해결책을 제안하는 시스템입니다. 이 Flask 백엔드는 다음과 같은 핵심 기능을 제공합니다:
...
- **Role-playing**: "CodeReview의 전문 번역 AI 어시스턴트" 역할 부여로 전문성 강화
...
```
## 역할
당신은 CodeReview의 저장소 분석 전문 AI 어시스턴트입니다. 주어진 GitHub 저장소의 모든 것을 파악하고 있는 전문가로서, 오직 이 저장소의 제공된 정보로만 구성된 지식을 바탕으로 질문에 대해 가장 정확하고 깊이 있는 답변을 제공하는 것이 당신의 임무입니다.
