...
class RepositoryService:
    ...
    def get_repository_status(self, repo_name):
        ...
        with self._status_lock:  # Lock으로 보호
            if repo_name not in self.repository_status:
                ...
                code_index_path = self._get_index_path(
                    repo_name, "code"
                )  # 헬퍼 함수 사용
                ...
                if code_exists or doc_exists:
                    ...
                    return {
                        ...
                        "repo_name": repo_name,
                        "code_index_status": (
                            "completed" if code_exists else "not_found"
                        ),
                        "document_index_status": (
                            "completed" if doc_exists else "not_found"
                        ),
    ...
    def _update_error_status(self, repo_name, error_msg, error_code=None):
        """인덱싱 오류 발생 시 상태를 'failed'로 업데이트합니다. (Lock 외부에서 호출되지만, 호출하는 쪽에서 Lock을 잡아야 함)"""
        if (
            repo_name in self.repository_status
        ):  # Lock 없이 읽기만 하는 것은 비교적 안전하나, 일관성을 위해 호출부에서 Lock
            current_time_iso = datetime.now(timezone.utc).isoformat()
            self.repository_status[repo_name].update(  # 실제 쓰기는 Lock 필요
                {
                    "status": "failed",
