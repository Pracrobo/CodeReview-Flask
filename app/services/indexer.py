...
class RepositoryIndexer:
    ...
    def create_indexes_from_repository(
        self, repo_url: str, progress_callback=None
    ) -> Dict[str, Optional[object]]:
        """저장소로부터 코드 인덱스를 생성합니다.
        ...
        Returns:
            생성된 코드 벡터 스토어 딕셔너리 {"code": vector_store}
        ...
        try:
            ...
            self.git_service.clone_or_load_repository(repo_url)
    ...
    def _create_code_index(
        ...
        if not self.document_loader.is_supported_language(primary_language):
            logger.warning(
                f"지원되지 않는 언어: {primary_language}. 코드 인덱싱을 건너뜁니다."
            )
            return None
...
def create_index_from_repo(
    repo_url: str, local_repo_path: str, embedding_model_name: str
) -> Dict[str, Optional[object]]:
