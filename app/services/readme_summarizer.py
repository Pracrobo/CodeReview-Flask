...
class ReadmeSummarizer:
    ...
    def _clean_readme_content(
        ...
        content = re.sub(
            r"(?i)## table of contents.*?(?=##|\Z)", "", content, flags=re.DOTALL
        )
        ...
        content = re.sub(
            r"(?i)## usage.*?(?=##|\Z)", "[사용법 생략]", content, flags=re.DOTALL
        )
        content = re.sub(
            r"(?i)## examples.*?(?=##|\Z)", "[예제 생략]", content, flags=re.DOTALL
        )
        content = re.sub(
            r"(?i)## contributing.*?(?=##|\Z)",
            "[기여 방법 생략]",
        ...
        if len(content) > 4000:
            ...
            logger.warning(
                f"'{repo_name}' 저장소의 README 내용이 기준 길이(4000자)를 초과하여 일부를 절단했습니다."
            )
                ...
                try:
                    ...
                    try:
                        ...
                        if hasattr(response, "to_dict"):
                            ...
                            logger.debug(
                                f"Gemini API 응답 사전(dict) 형태: {response_dict}"
                            )
                        elif hasattr(response, "__dict__"):
                            logger.debug(
                                f"Gemini API 응답 __dict__: {response.__dict__}"
                            )
                    except Exception as debug_e:
                        logger.debug(
                            f"Gemini API 응답 구조 디버깅 중 오류 발생: {debug_e}"
                        )
                    ...
                    if hasattr(response, "text") and response.text:
                        ...
                        logger.debug(
                            f"요약 추출 성공 (text 속성 직접 접근): {len(summary)}자"
                        )

                    # 방법 2: candidates 구조에서 추출
                    elif hasattr(response, "candidates") and response.candidates:
                        logger.debug(
                            f"API 응답 'candidates' 수: {len(response.candidates)}"
                        )
                        ...
                        for i, candidate in enumerate(response.candidates):
                            ...
                            if hasattr(candidate, "content") and candidate.content:
                                logger.debug(
                                    f"Candidate {i} content 객체 속성: {dir(candidate.content)}"
                                )
                                ...
                                if (
                                    ...
                                    logger.debug(
                                        f"Candidate {i} content.parts 수: {len(candidate.content.parts)}"
                                    )
                                    ...
                                    for j, part in enumerate(candidate.content.parts):
                                        ...
                                        if hasattr(part, "text") and part.text:
                                            ...
                                            logger.debug(
                                                f"요약 추출 성공 (candidate {i} part {j} 접근): {len(summary)}자"
                                            )
                                            break
                                ...
                                # content에 직접 text가 있는 경우
                                    ...
                                    logger.debug(
                                        f"요약 추출 성공 (candidate {i} content.text 접근): {len(summary)}자"
                                    )
                                    break
                            ...
                            # candidate에 직접 text가 있는 경우
                                ...
                                logger.debug(
                                    f"요약 추출 성공 (candidate {i}.text 접근): {len(summary)}자"
                                )
                                break
                    ...
                    if not summary:
                        try:
                            ...
                            logger.debug(
                                f"Gemini API 응답 문자열 (일부): {response_str[:500]}"
                            )
                            ...
                            if text_match:
                                ...
                                logger.debug(
                                    f"요약 추출 성공 (정규식 text 패턴): {len(summary)}자"
                                )

                            # 'text': '내용' 패턴 찾기
                            elif re.search(r"'text':\s*'([^']+)'", response_str):
                                text_match = re.search(
                                    r"'text':\s*'([^']+)'", response_str
                                )
                                summary = text_match.group(1).strip()
                                logger.debug(
                                    f"요약 추출 성공 (정규식 text 패턴, 단일 따옴표): {len(summary)}자"
                                )

                        except Exception as str_e:
                            logger.debug(
                                f"Gemini API 응답 문자열 파싱 중 오류: {str_e}"
                            )
                            ...
                            if hasattr(response, "parts") and response.parts:
                                for part in response.parts:
                                    if hasattr(part, "text") and part.text:
                                        summary = part.text.strip()
                                        logger.debug(
                                            f"요약 추출 성공 (response.parts 직접 접근): {len(summary)}자"
                                        )
                                        break
                        except Exception as parts_e:
                            logger.debug(
                                f"Gemini API 응답 response.parts 접근 중 오류: {parts_e}"
                            )
