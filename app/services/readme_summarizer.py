    ...
    def summarize_readme(self, repo_name: str, readme_content: str) -> Optional[str]:
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(
                        f"README 요약 시도 {attempt + 1}/{self.max_retries}: {repo_name}"
                    )
                    ...
                    # 방법 2: candidates 구조에서 추출
                        ...
                        for i, candidate in enumerate(response.candidates):
                            ...
                            if hasattr(candidate, "content") and candidate.content:
                                logger.debug(
                                    f"candidate {i} content 속성들: {dir(candidate.content)}"
                                )
                                ...
                                if (
                                    ...
                                    logger.debug(
                                        f"candidate {i} parts 수: {len(candidate.content.parts)}"
                                    )
                                    ...
                                    for j, part in enumerate(candidate.content.parts):
                                        ...
                                        if hasattr(part, "text") and part.text:
                                            ...
                                            logger.debug(
                                                f"방법 2 성공 - candidate {i} part {j}: {len(summary)}자"
                                            )
                                            break
                                ...
                                # content에 직접 text가 있는 경우
                                    ...
                                    logger.debug(
                                        f"방법 2-2 성공 - candidate {i} content.text: {len(summary)}자"
                                    )
                                    break
                            ...
                            # candidate에 직접 text가 있는 경우
                                ...
                                logger.debug(
                                    f"방법 2-3 성공 - candidate {i}.text: {len(summary)}자"
                                )
                                break
                    ...
                    if not summary:
                        try:
                            ...
                            logger.debug(
                                f"응답 문자열 (처음 500자): {response_str[:500]}"
                            )
                            ...
                            if text_match:
                                ...
                                logger.debug(
                                    f"방법 3-1 성공 - 정규식 text 패턴: {len(summary)}자"
                                )
                            ...
                            # 'text': '내용' 패턴 찾기
                                ...
                                summary = text_match.group(1).strip()
                                logger.debug(
                                    f"방법 3-2 성공 - 정규식 text 패턴 (단일 따옴표): {len(summary)}자"
                                )
                            ...
                            if hasattr(response, "parts") and response.parts:
                                for part in response.parts:
                                    if hasattr(part, "text") and part.text:
                                        summary = part.text.strip()
                                        logger.debug(
                                            f"방법 4 성공 - 직접 parts: {len(summary)}자"
                                        )
                                        break
                    ...
                    if summary:
                        ...
                        logger.warning(
                            f"응답에서 텍스트를 추출할 수 없습니다: {repo_name}"
                        )
                        # 실제 응답 내용을 더 자세히 로그
                    ...
                    logger.warning(
                        f"README 요약 시도 {attempt + 1} 실패: {repo_name} - {e}"
                    )
