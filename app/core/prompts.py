...
class PromptTemplates:
    ...
    def get_code_query_translation_prompt(korean_text: str) -> str:
        ...
        return f"""
## 역할
당신은 한국어와 영어에 모두 능통한 전문 이중언어 프로그래머입니다. 당신의 임무는 프로그래밍에 관한 한국어 질문을 영어로 번역하는 것입니다.
...
"""
    ...
    def get_code_rag_prompt(context: str, question: str) -> str:
        ...
        return f"""
## 역할
당신은 10년차 시니어 개발자이자, 복잡한 코드도 쉽게 설명하는 기술 블로그 전문가입니다. 당신의 임무는 주어진 '코드 컨텍스트'만을 활용하여 질문에 명확하고 친절하게 답변하는 것입니다.
...
"""
    ...
    def get_readme_summary_prompt(repo_name: str, readme_content: str) -> str:
        ...
        return f"""
## 역할
당신은 다른 개발자들이 프로젝트를 빠르고 정확하게 파악할 수 있도록, GitHub README 문서를 분석하고 핵심 정보를 추출하여 요약하는 15년차 시니어 소프트웨어 엔지니어입니다.
...
"""
    ...
    def get_issue_to_query_prompt(issue_title: str, issue_body: str) -> str:
        ...
        return f"""
## 역할
당신은 GitHub 이슈만 보고도 문제의 원인이 되는 코드를 정확히 찾아내는 20년차 '디버깅 전문가'입니다. 당신의 임무는 주어진 이슈 내용을 분석하여, 문제 해결에 가장 적합한 코드를 찾을 수 있는 간결하고 정밀한 영어 검색 질문(쿼리)을 만드는 것입니다.
...
"""
    ...
    def get_ai_solution_suggestion_prompt(
        ...
        return f"""
## 역할
당신은 20년 경력의 '수석 소프트웨어 아키텍트'이자, 복잡한 시스템의 버그를 분석하고 최적의 해결책을 제시하는 전문가입니다. 당신의 임무는 주어진 모든 정보를 종합하여, 개발자가 즉시 검토하고 적용할 수 있는 수준의 구체적이고 안정적인 해결 제안을 작성하는 것입니다.
...
"""
    ...
    def get_issue_summary_prompt(issue_title: str, issue_body: str) -> str:
        ...
        return f"""
## 역할
당신은 매일 수십 개의 기술 이슈를 빠르고 정확하게 파악하여 팀에 공유해야 하는 '테크 리드(Tech Lead)'입니다. 당신의 임무는 주어진 이슈의 핵심을 개발자가 즉시 이해할 수 있도록 명확하고 간결하게 요약하는 것입니다.
...
"""
    ...
    def get_repository_context_answer_prompt(
        ...
        return f"""
## 역할
당신은 주어진 GitHub 저장소 '{repo_name}'의 모든 것을 파악하고 있는 'AI 수석 엔지니어'입니다. 당신의 지식은 오직 이 저장소의 제공된 정보로만 구성되어 있으며, 이 정보를 바탕으로 질문에 대해 가장 정확하고 깊이 있는 답변을 제공해야 합니다.
